...
class myCNN():
    ...
    def CNN(input):
        ...
        #print(model.output_shape)
        model.add(Conv1D(filters = 64, kernel_size = 1,padding='same',activation='relu',kernel_initializer='uniform'))
        model.add(MaxPooling1D(pool_size=1, padding='same')) 
        model.add(Conv1D(filters = 32, kernel_size = 1,padding='same',activation='relu',kernel_initializer='uniform'))
        model.add(MaxPooling1D(pool_size=1, padding='same')) 
        #model.add(BatchNormalization(momentum=0.9))
        model.add(Conv1D(filters = 16, kernel_size = 1,padding='same',activation='relu',kernel_initializer='uniform'))
        model.add(MaxPooling1D(pool_size=1, padding='same')) 
        #print(model.output_shape)
    ...
    def train(x_train,y_train,x_test, num_classes,test_size):
        ...
        #print(y_train)
        model = myCNN.CNN(x_train,num_classes)
